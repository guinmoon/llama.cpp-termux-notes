#!/bin/bash
LLAMACPP_PATH="/data/data/com.termux/files/home/llama.cpp-termux-notes/llama.cpp"
# Validate arguments
if [ $# -lt 2 ]; then
    echo "Error: Two arguments required: model and type (cpu/openblas/blis/kleidi/openblas_kle/opencl/vk_opencl_openblas/vk)"
    echo "Example: $0 $LLAMACPP_PATH/models/llama-7b.bin blis"
    exit 1
fi
...
if [[ "$TYPE" == *"openblas_kle"* ]]; then
    ...
    $LLAMACPP_PATH/build_openblas_kle/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1
    sleep 90
    echo "Running OpenBLAS-KLE benchmarks (cores 4,5,6,7)"
    taskset -c 4,5,6,7 $LLAMACPP_PATH/build_openblas_kle/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1
    sleep 90
    echo "Running OpenBLAS-KLE benchmarks (cores 0,1,2,3)"
    taskset -c 0,1,2,3 $LLAMACPP_PATH/build_openblas_kle/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1
elif [[ "$TYPE" == *"blis"* ]]; then
    echo "Running BLIS benchmarks (all cores)"
    $LLAMACPP_PATH/build_blis/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1
    sleep 90
    echo "Running BLIS benchmarks (cores 4,5,6,7)"
    taskset -c 4,5,6,7 $LLAMACPP_PATH/build_blis/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1
    sleep 90
    echo "Running BLIS benchmarks (cores 0,1,2,3)"
    taskset -c 0,1,2,3 $LLAMACPP_PATH/build_blis/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1
elif [[ "$TYPE" == *"kleidi"* ]]; then
    echo "Running KLEIDI benchmarks (all cores)"
    $LLAMACPP_PATH/build_kleidi/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1
    sleep 90
    echo "Running KLEIDI benchmarks (cores 4,5,6,7)"
    taskset -c 4,5,6,7 $LLAMACPP_PATH/build_kleidi/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1
    sleep 90
...
#     LD_LIBRARY_PATH=$LLAMACPP_PATH/:$PREFIX/lib taskset -c 0,1,2,3 $LLAMACPP_PATH/build_vk_opencl_openblas/bin/llama-bench -m "$MODEL" -t 3 -fa 1 -ngl 99 --device GPUOpenCL
    ...
    LD_LIBRARY_PATH=$LLAMACPP_PATH/:$PREFIX/lib taskset -c 4,5,6,7 $LLAMACPP_PATH/build_opencl/bin/llama-bench -m "$MODEL" -t 3  -fa 1
    sleep 90
    ...
    taskset -c 4,5,6,7 $LLAMACPP_PATH/build_vk/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1 -ngl 99
    sleep 90
    echo "Running Vulkan benchmarks (cores 0,1,2,3)"
    taskset -c 0,1,2,3 $LLAMACPP_PATH/build_vk/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1 -ngl 99
elif [[ "$TYPE" == *"cpu"* ]]; then
    echo "Running CPU benchmarks (all cores)"
    $LLAMACPP_PATH/build_cpu/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1
    sleep 90
    echo "Running CPU benchmarks (cores 4,5,6,7)"
    taskset -c 4,5,6,7 $LLAMACPP_PATH/build_cpu/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1
    sleep 90
    echo "Running CPU benchmarks (cores 0,1,2,3)"
    taskset -c 0,1,2,3 $LLAMACPP_PATH/build_cpu/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1
elif [[ "$TYPE" == *"openblas"* ]]; then
    echo "Running OpenBLAS benchmarks (all cores)"
    $LLAMACPP_PATH/build_openblas/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1
    sleep 90
    echo "Running OpenBLAS benchmarks (cores 4,5,6,7)"
    taskset -c 4,5,6,7 $LLAMACPP_PATH/build_openblas/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1
    sleep 90
    echo "Running OpenBLAS benchmarks (cores 0,1,2,3)"
    taskset -c 0,1,2,3 $LLAMACPP_PATH/build_openblas/bin/llama-bench -m "$MODEL" -t 3 -ctk q8_0 -ctv q8_0 -fa 1
else
    echo "Error: Unknown build type '$TYPE'. Supported types: cpu, openblas, blis, kleidi, openblas_kle, opencl, vk_opencl_openblas, vk"
    exit 1
fi

LD_LIBRARY_PATH=$LLAMACPP_PATH/:$PREFIX/lib $LLAMACPP_PATH/build_opencl/bin/llama-bench -m ~/storage/downloads/gemma-3-1b-it-q4_0-pure.gguf -ngl 26 -t 3


LD_LIBRARY_PATH=$LLAMACPP_PATH/:$PREFIX/lib $LLAMACPP_PATH/build_vk_opencl/bin/llama-cli -m ~/storage/downloads/gemma-3-4b-it-q4_0-pure.gguf -ngl 34 -t 3 --main-gpu 1 -dev Vulkan0,GPUOpenCL --tensor-split 14,20 


$LLAMACPP_PATH/build_vk_opencl_openblas/bin/llama-cli -m ~/storage/downloads/gemma-3-4b-it-q4_0-pure.gguf -ngl 0 -t 3 -dev none -ctk q8_0 -ctv q8_0 -p "who are you?"
...
# LD_LIBRARY_PATH=$LLAMACPP_PATH/:$PREFIX/lib $LLAMACPP_PATH/build_vk_opencl_openblas/bin/llama-cli --list-devices
